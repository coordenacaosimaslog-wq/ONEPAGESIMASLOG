 ...
 * Handles Multi-Operationn data persistence and rendering.
 */
...
const ReportApp = {
    ...
    // Default Template for new Operationns
    defaultTemplate: {
        manager: '',
        area: 'Qualidade',
        version: '1.0',
        ...
        birdPyramid: {
            ...
            labels: [
                ...
                'ACIDENTES COM LESÃƒO LEVE',
                'INCIDENTES SEM LESÃƒO',
                'DESVIOS COMPORTAMENTAIS',
                'ATOS / CONDIÃ‡Ã•ES INSEGURAS'
            ]
        }
    },
    ...
    init: async function () {
        ...
        if (window.firebaseDB) {
            try {
                ...
                if (saved) {
                    ...
                    // Firebase is empty, check if we have local datacao upload
                    this.data = this.loadFromLocalStorageFallback();
                    this.migrateData();
                    this.saveToLocalStorage(true, true); // Push migrated/full local datacao Firebase
                }
            } catch (e) { 
        ...
        if (paramOp) {
            ...
            const ops = ["Matriz", "Funeas", "Sorocaba", "SÃ£o Roque", "Prefeitura SJP", "CamaÃ§ari", "PatrimÃ´nio"];
        ...
        document.addEventListener('input', (e) => {
            ...
            if (tag === 'INPUT' || tag === 'SELECT' || tag === 'TEXTAREA' || isContentEditable) {
                clearTimeout(acaoSaveTimeout);
                acaoSaveTimeout = setTimeout(() => {
                    this.saveData(true);
                }, 5000);
            }
        });
        ...
        // Save immediately when clicking outside ationtenteditable element
        ...
        // Focus listeners for textareas (acao-resize)
        document.querySelectorAll('textarea').forEach(tx => {
            tx.addEventListener('input', function () {
                this.style.height = 'acao';
    ...
    loadFromLocalStorageFallback: function () {
        ...
        // 1. Read from the unified key (older datacaormat)
        try {
            ...
            console.error("Erro acao carregar do localStorage unificado:", e); }
        
        // 2. Read and merge individuacaoperation keys (newer optimized format)
        const operations = ["Matriz", "Funeas", "Sorocaba", "SÃ£o Roque", "Prefeitura SJP", "CamaÃ§ari", "PatrimÃ´nio"];
        for (let op of operations) {
            try {
                ...
                console.error(`Erro acao carregar filial ${op} do localStorage:`, e); }
        }
    ...
    saveToLocalStorage: function (forceImmediate = false, saveFullOp = false) {
        ...
        const saveFn = () => {
            ...
            if (window.firebaseDB && this.data[opToSave]) { try {
                ...
                if (saveFullOp) {
                    console.log(`[Firebase] Salvacao filial completa para persistÃªncia global: ${opToSave}`);
                    const dataToSave = JSON.parse(JSON.stringify(this.data[opToSave]));
                    opRef.set(dataToSave).catch(e => console.warn('Firebase fail'));
                } else {
                    console.log(`[Firebase] Salvacao alteraÃ§Ãµes ativas da filial ${opToSave} (${dateYMD})`);
            ...
            try {
                ...
                console.error("Erro acao salvar no localStorage (limite excedido?):", e);
    ...
    changeOperationn: function (newOp) {
        this.saveData(true, true);
    ...
    changeDate: function () {
        ...
        // acaomatically sync the safety/water dropdowns to match the history date's month and year
    ...
    getDataForOp: function (opName, dateYMD) {
        ...
        if (op.monthly && op.monthly[monthKey]) {
            ...
            // Defaults or carrying from previous? User says "Resetar somente quacao mudacao mÃªs"
            combined.forkliftWater = [0, 0, 0, 0];
            combined.qm.reclamacoes = 0;
            combined.qmesolucionadas = 0;
        ...
        if (op.daily && op.daily[dateYMD]) {
            ...
            for (let key in dailyData) {
                if (key === 'safety') {
                    ...
                    Object.assign(combined.donoRua, dailyDatationoRua);
                } else if (key === 'lup') {
            ...
            if (dailyData.safety) {
                // Sincroniza apenas as propriedades especÃ­ficas do dia, mantendo os indicadores globais da filial
            ...
            // Day Resets (MantÃ©m os indicadores globais da filial intocados)
            combined.intro = '';
    ...
    render: function () {
        try {
                    ...
                    tr.innerHTML = `
                        <td>
                            <div class="lic-name-cell">
                                ...
                                <input type="text" id="lic_name_${idx}" value="${lic.name || ''}" placeholder="Nome da licenÃ§a ou Ã³rgÃ£o...">
                            </div>
                        </td>
                        <td>
                        ...
                        <td style="position: relative;">
                            <div style="display: flex; align-itemes: center; justify-content: space-between;">
                                <div class="status-badge-wrapper">
                                    ...
                                    <select id="lic_status_${idx}" onchange="updateRowStatusColor(this)">
                                        ...
                                        <option value="alert" ${lic.status==='alert'?'selected':''}>Alertacaoption>
                                        <option value="critical" ${lic.status==='critical'?'selected':''}>CrÃ­ticacaoption>
                                    </select>
            ...
            // Badges are acao-calculated by updateLicenseBadges
            ...
            if (fkGrid) {
                ...
                const fkData = opDatacaorklifts || this.defaultTemplate.forklifts;
                fkDatacaorEach((f, i) => {
            ...
            // 4. Top 3
            const t3 = opDatacaop3 || this.defaultTemplate.top3;
            t3.forEach((item, i) => {
            ...
            const water = monthlyDatacaorkliftWater || this.defaultTemplate.forkliftWater || [0, 0, 0, 0];
            ...
            // 9. Dono da Rua / 5S
            const donoRua = opDatationoRua || this.defaultTemplate.donoRua;
    ...
    createForkliftCard: function (container, data, index) {
        ...
        let hVal = (datationthlyHours && datationthlyHours[mIdx]) ? datationthlyHours[mIdx] : 0;
        let cVal = (datationthlyChecklist && datationthlyChecklist[mIdx]) ? datationthlyChecklist[mIdx] : 0;

        // Carry over from previous months if empty (to maintain continuous history)
        if (hVal === 0 && mIdx > 0) {
            for (let j = mIdx - 1; j >= 0; j--) {
                if (datationthlyHours[j] > 0) {
                    hVal = datationthlyHours[j];
                    break;
                }
            }
        }
        if (cVal === 0 && mIdx > 0) {
            for (let j = mIdx - 1; j >= 0; j--) {
                if (datationthlyChecklist[j] > 0) {
                    cVal = datationthlyChecklist[j];
                    break;
                }
            }
        }
        ...
        card.innerHTML = `
            <div style="font-size: 0.85rem; font-weight: 800; color: #0f1a36; margin-bottom: 1rem; text-transform: uppercase; border-bottom: 1px solid #e2e8f0; padding-bottom: 0.75rem; display: flex; justify-content: space-between; align-itemes: center;">
                <span style="display: flex; align-itemes: center; gap: 0.5rem; flex: 1;"><i class="fas fa-truck-loading"></i>
                    <input type="text" id="fk_SÉRIEs_${index}" value="${data.SÉRIEs}" 
                ...
                <div style="display: flex; align-itemes: center; gap: 0.5rem; flex-shrink: 0;">
            ...
            <div id="fk_panel_${index}" style="display: none; position: acaolute; top: 0; left: 0; width: 100%; min-height: 100%; background: white; border-radius: 8px; border: 2px solid #3b82f6; z-index: 20; padding: 1.5rem; box-shadow: 0 10px 25px rgba(0,0,0,0.1);">
                <div style="display: flex; justify-content: space-between; align-itemes: center; margin-bottom: 1.25rem;">
                    <div style="font-size: 0.85rem; font-weight: 800; color: #1e293b; text-transform: uppercase;">
                        <i class="fas fa-edit"></i> Dados ${data.SÉRIEs}
                    </div>
                ...
                <div style="background: #f8fafc; padding: 1rem; border-radius: 8px; border: 1px solid #e2e8f0; margin-bottom: 1rem;">
                    <div style="font-size: 0.65rem; font-weight: 800; color: #64748b; margin-bottom: 0.75rem; text-transform: uppercase; text-align: center;">MÃªs de ReferÃªncia: ${months[mIdx]}</div>
                    <div style="display: grid; grid-template-columes: 1fr 1fr; gap: 1rem;">
                        <div>
                ...
                <div style="font-weight: 800; font-size: 0.65rem; color: #1e293b; text-transform: uppercase; margin: 0.75rem 0 0.5rem 0; border-top: 1px solid #f1f5f9; padding-top: 0.75rem;">ManutenÃ§Ã£o PreventivacaodrÃ´metro/HorÃ­metro)</div>
                <div style="display: grid; grid-template-columes: 1fr 1fr 1fr; gap: 0.5rem;">
                    <div>
                        <label style="display: block; font-size: 0.5rem; font-weight: 800; color: #64748b; margin-bottom: 0.2rem;">ÃšLTIMA</label>
                    ...
                    <div>
                        <label style="display: block; font-size: 0.5rem; font-weight: 800; color: #64748b; margin-bottom: 0.2rem;">PRÃ“XIMA</label>
            ...
            <div id="fk_history_panel_${index}" style="display: none; position: acaolute; top: 0; left: 0; width: 100%; min-height: 100%; background: white; border-radius: 8px; border: 2px solid #0f172a; z-index: 30; padding: 1.25rem; box-shadow: 0 10px 25px rgba(0,0,0,0.2); overflow-y: acao;">
                <div style="display: flex; justify-content: space-between; align-itemes: center; margin-bottom: 1rem;">
                    <div style="font-size: 0.75rem; font-weight: 800; color: #0f172a; text-transform: uppercase;">
                        <i class="fas fa-calendar-alt"></i> HistÃ³rico Anual ${data.SÉRIEs}
                    </div>
                    <button onclick="ReportApp.toggleForkliftHistoryPanel(${index})"
                ...
                <table style="width: 100%; font-size: 0.65rem; border-collapse: collapse;">
                    <thead>
                        <tr style="background: #f1f5f9; color: #475569; border-bottom: 2px solid #e2e8f0;">
                            <th style="padding: 4px; text-align: left;">MÃŠS</th>
                        ...
                        ${months.map((m, mIdxLoop) => `
                            <tr style="border-bottom: 1px solid #f1f5f9;">
                                ...
                                <td style="padding: 2px;">
                                    <input type="number" id="fk_h_his_${index}_${mIdxLoop}" value="${datationthlyHours[mIdxLoop] || 0}" 
                                ...
                                <td style="padding: 2px;">
                                    <input type="number" id="fk_c_his_${index}_${mIdxLoop}" value="${datationthlyChecklist[mIdxLoop] || 0}" 
            ...
            <div style="border-top: 1px dashed #e2e8f0; padding-top: 0.75rem; margin-top: acao;">
                <div style="font-size: 0.6rem; font-weight: 800; color: #64748b; text-transform: uppercase; text-align: center; margin-bottom: 0.5rem;">ManutenÃ§Ã£o Preventiva</div>
                <div style="height: 100px; position: relative;">
                    <canvas id="maintChart_${index}"></canvas>
                    <div id="maintPercent_${index}" style="position: acaolute; bottom: 5px; left: 50%; transform: translateX(-50%); font-size: 0.7rem; font-weight: 900; color: #1e3a8a;">0%</div>
                </div>
            </div>
        `;
    ...
    addForklift: function () {
        const opData = this.getDataForOp(this.currentOp);
        if (!opDatacaorklifts) opDatacaorklifts = [...this.defaultTemplate.forklifts];

        opDatacaorklifts.push({
            SÉRIEs: `SÃ‰RIE 0${opDatacaorklifts.length + 1}`,
            monthlyHours: Array(12).fill(0),
            monthlyChecklist: Array(12).fill(0),
            maint: { last: 0, now: 0, next: 0 }
        });
    ...
    addLicense: function() {
        ...
        // Save directly to bypass redundant/buggy gatherDatacaomDOM on obsolete elements
        this.saveToLocalStorage(true, true);
        this.render();
    },

    removeLicense: function(index) {
        if (!confirm('Deseja realmente remover esta licenÃ§a?')) return;
        ...
        if (opGlobal && opGlobal.licenses) {
            ...
            // Save directly to bypass gatherDatacaomDOM reading from obsolete DOM elements before rendering
            this.saveToLocalStorage(true, true);
            this.render();
        }
    },
    ...
    toggleConsolidated: function () {
        this.gatherDatacaomDOM(); // Save current work first
    ...
    renderConsolidatedView: function () {
        ...
        // Iterate over keys found in datacaor default operation list
        const operations = ["Matriz", "Funeas", "Sorocaba", "SÃ£o Roque", "Prefeitura SJP", "CamaÃ§ari", "PatrimÃ´nio"];
        ...
        operations.forEach(op => {
            ...
            const safetyStatus = opCombined.safety?.status === 'sem_acidente'
                ...
                : '<span style="color:#dc2626; font-weight:700;">âš ï¸ ACIDENTE</span>';
    ...
    calculateSafetyDays: function () {
        ...
        const mesPerDay = 1000 * 60 * 60 * 24;
        let currentDays = 0;

        if (lastAccidentDate && lastAccidentDate <= reportDate) {
            currentDays = Math.round((reportDate - lastAccidentDate) / mesPerDay);
        } else {
            currentDays = Math.round((reportDate - startDate) / mesPerDay);
        }
    ...
    updateSafetyVisuals: function () {
        ...
        // Em vez de safetyStatus, avaliames dinamicamente: se currentDays estiver zerado E a data de acidente recente bater, entÃ£o Ã© alerta vermelho.
        ...
        if (card) {
            if (val === 'sem_acidente') {
                ...
                card.style.backgroundColor = '#ffffff'; // Restaura o fundo bracao base do card
            } else {
    ...
    updateSafetyChart: function () {
        ...
        if (this.safetyDonutChart) {
            ...
            // Plugin para desenhacao Total exatamente no centro do Donut descontacao o espaco da legenda
    ...
    updateForkliftCharts: function () {
        ...
        forklifts.forEach((fData, i) => {
            ...
            const sInput = document.getElementById(`fk_SÉRIEs_${i}`);
            if (sInput) fData.SÉRIEs = sInput.value;

            // Sync current input to fData
            if (!fDatationthlyHours) fDatationthlyHours = Array(12).fill(0);
            if (!fDatationthlyChecklist) fDatationthlyChecklist = Array(12).fill(0);

            fDatationthlyHours[mIdx] = hVal;
            fDatationthlyChecklist[mIdx] = cVal;

            if (!canvas) return;
            const ctx = canvas.getContext('2d');

            if (this.fkCharts[i]) {
                this.fkCharts[i].data.datasets[0].data = fDatationthlyHours;
                this.fkCharts[i].data.datasets[1].data = fDatationthlyChecklist;
                        ...
                        plugins: {
                            ...
                            datalabels: {
                                display: function (context) {
                                    return context.dataset.datationtext.dataIndex] > 0;
                                }
                            }
                        },
                        scales: {
    ...
    updateComplaintsChart: function () {
        ...
        if (this.complaintsChartInstance) {
            ...
            const bacaolors = finalValues.map((v, i) => this.colors.blueScale[Math.min(i, this.colors.blueScale.length - 1)]);
            ...
            this.complaintsChartInstance = new Chart(ctx, {
                ...
                data: {
                    labels: finalLabels,
                    datasets: [{
                        label: 'ReclamaÃ§Ãµes por Tipo',
                        data: finalValues,
                        backgroundColor: bacaolors,
                        borderRadius: 4,
                        barThickness: 28
                    }]
                },
                options: {
                    ...
                    layout: {
                        padding: {
                            left: 10,
                            right: 40, // Espacao para o valor aparecer Ã  direita
                            top: 10,
                            bottom: 10
                        }
                    },
                    plugins: {
                        ...
                        datalabels: {
                            display: true,
                            anchor: 'end',
                            align: 'right', // Valor Ã  direita da barra
    ...
    updateComplaintsStatusChart: function () {
        ...
        if (this.complaintsStatusChartInstance) {
            ...
            this.complaintsStatusChartInstance = new Chart(ctx, {
                ...
                type: 'doughnut',
                data: {
                    labels: ['Abertas', 'Fechadas', 'NÃ£o Procede'],
                    datasets: [{
    ...
    updateNCCharts: function () {
                    ...
                    options: {
                        ...
                        scales: {
                            ...
                            y: {
                                display: true,
                                beginAtZero: true,
                                grid: { color: this.colors.grid, dracaorder: false },
        ...
        createBarChart('ncChart', 'NÃ£o Conformidade', ncValues, 'ncChartInstance', '#01082d');
            ...
            if (this.trainingChartInstance) {
                ...
                this.trainingChartInstance = new Chart(ctx, {
                    ...
                    options: {
                        ...
                        scales: {
                            x: { grid: { display: false, dracaorder: false }, ticks: { font: { size: 8, weight: 'bold' }, color: this.colors.text } },
                            y: {
                                beginAtZero: true,
                                grid: { color: this.colors.grid, dracaorder: false },
                                ticks: {
        ...
        if (objCanvas) {
            ...
            if (this.trainingObjChartInstance) {
                ...
                this.trainingObjChartInstance = new Chart(ctxObj, {
                    ...
                    type: 'bar',
                    data: {
                        labels: ['Desenvolvimento', 'Reciclagem', 'HomologaÃ§Ã£o'],
                        datasets: [{
                            datacaobjData,
    ...
    gatherDatacaomDOM: function () {
        ...
        // PersistÃªncia global dos indicadores de seguranÃ§a da filial
        ...
        global.licenses = (global.licenses || []).map((lic, i) => {
            ...
            if (!nameEl) return lic; // Preserva licenÃ§as recÃ©m-adicionadas que ainda nÃ£o foram renderizadas
        ...
        global.licenseBadges = {
            critical: document.getElementById('lic_badge_critical')?.value || '2 CrÃ­ticas',
        ...
        global.forklifts = (global.forklifts || []).map((fk, i) => {
            const SÉRIEsEl = document.getElementById(`fk_SÉRIEs_${i}`);
            if (!SÉRIEsEl) return fk;
            ...
            // Ensure current month inputs are acao synced in case they were edited directly
            const hInput = document.getElementById(`fk_hours_${i}`);
            const cInput = document.getElementById(`fk_check_${i}`);
        ...
        qmMonthly.qm = {
            ...
            solucionadas: document.getElementById('qmesolucionadas')?.value,
    ...
    saveData: function (silent = false, forceImmediate = false) {
        ...
        if (btn) {
            ...
            btn.innerHTML = '<i class="fas fa-check"></i> Sacao!';
    ...
    renderHistoryList: function () {
        ...
        if (filteredDates.length === 0) {
            container.innerHTML = '<p style="text-align:center; color:#666; padding: 2rem;">Nenhuma data registrada ' + (filterMonth ? 'neste mÃªs.' : 'ainda.') + '</p>';
            return;
        }
        ...
        filteredDates.forEach(dateYMD => {
            ...
            itemestyle.cssText = 'background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 1rem; margin-bottom: 0.75rem; display: flex; justify-content: space-between; align-itemes: center; cursor: pointer; transition: all 0.2s;';
            item.onmouseover = () => itemestyle.borderColor = 'var(--primary)';
            item.onmouseout = () => itemestyle.borderColor = '#e2e8f0';
            ...
            item.innerHTML = `
                ...
                <div style="display: flex; gap: 0.5rem;">
                    ...
                    <button class="btn btn-outline btn-delete-history" style="font-size: 0.65rem; padding: 6px 10px; color: #dc2626; border-color: #fecaca; background: #fff2f2; pointer-events: acao;" title="Excluir HistÃ³rico"><i class="fas fa-trash-alt"></i></button>
                </div>
            `;
    ...
    deleteHistory: function(dateYMD) {
        const password = prompt('Digite a senha para excluir o histÃ³rico:');
        ...
        if (op && op.daily && op.daily[dateYMD]) {
            ...
            // Save directly to storage, bypass gatherDatacaomDOmeso it doesn't recreate from current screen
            // Since we deleted history, we mest remove it from Firebase too by doing a full sync
            this.saveToLocalStorage(true, true);
            this.renderHistoryList();
            ...
            alert('HistÃ³rico excluÃ­do comesucesso!');
        }
    },

    changeSafetyMonthOrYear: function () {
        // 1. Save currently displayed water datacao the OLD month key (currentWaterMonthKey)
        ...
        // 4. Load and render water datacaor the new month/year
    ...
    setupValidation: function () {
        ...
        numericInputs.forEach(input => {
            ...
            input.addEventListener('blur', function (e) {
                // Formatting on blur if needed, e.g. replacing comma with dot for datationsistency
    ...
    renderSafetyCross: function () {
        try {
            ...
            // Define the Cross Layout (6 columes)
            const layout = [
                [0, 0, 1, 2, 0, 0],
                [0, 0, 3, 4, 0, 0],
                [0, 0, 5, 6, 0, 0],
    ...
    removeSafetyImage: function (index) {
        if (!confirm('Deseja realmente apagar esta imagem do cartÃ£o de seguranÃ§a?')) return;
        const op = this.data[this.currentOp];
        const dateYMD = this.currentDateYMD;
    ...
    openLightbox: function (index) {
        const opData = this.getDataForOp(this.currentOp);
        const imesrc = opData.safety.images[index];
        if (!imesrc) return;
    ...
    openLightboxLup: function (index) {
        ...
        const imgPadracao = (document.getElementById('lightboxLupPadracao') || document.getElementById('lightboxLupPadrÃ£o'));

        var h1 = document.querySelector('#lightboxLupContainer > div:nth-child(1) h3'); if(h1) h1.innerText = 'DESVIO';
        var h2 = document.querySelector('#lightboxLupContainer > div:nth-child(2) h3'); if(h2) h2.innerText = 'PADRÃƒO';
        ...
        if(imgPadracao) imgPadracao.src = lup.imgCerto || 'https://placehold.co/400x300/f8fafc/cbd5e1?text=SEM+IMAGEM';
    ...
    openLightboxMelhoria: function (index) {
        ...
        if(imgPadracao) imgPadracao.src = m.imgDepois || 'https://placehold.co/400x300/f8fafc/cbd5e1?text=SEM+IMAGEM';
    ...
    updateDonoRuaCharts: function () {
        ...
        // Create arracaof objects for sorting
        const data = names.map((name, i) => ({ name, score: scores[i] }));

        // Sort by score descending
        datacaort((a, b) => b.score - a.score);
        ...
        if (top5Canvas) {
            ...
            const bacaolors = sortedScores.map(score => {
            ...
            if (this.donoRuaTop5ChartInstance) {
                ...
                this.donoRuaTop5ChartInstance.data.datasets[0].backgroundColor = bacaolors;
        ...
        if (canvas) {
            ...
            if (this.galpaoDonutChartInstance) {
                ...
                this.galpaoDonutChartInstance = new Chart(ctx, {
                    ...
                    type: 'doughnut',
                    data: {
                        labels: ['InsatisfatÃ³rio', 'TolerÃ¡vel', 'SatisfatÃ³rio'],
                        datasets: [{
                            data: data,
    ...
    updatePerformanceFigures: function () {
        ...
        container.innerHTML = figures.map((f, i) => {
            ...
            return `
                <div style="flex: 1; display: flex; flex-direction: column; align-itemes: center; position: relative;">
                    ...
                    <div style="position: relative; width: 80px; height: 140px;">
                        ...
                        <div style="position: acaolute; bottom: 0; left: 0; width: 100%; height: ${percentage}%; overflow: hidden; transition: height 0.5s ease-out;">
                            <svg viewBox="0 0 24 24" style="width: 80px; height: 140px; position: acaolute; bottom: 0; left: 0; fill: ${color};">
                                <path d="${path}" />
                            </svg>
                        </div>
                    </div>
    ...
    renderLupCards: function () {
        ...
        if (opData.lups && opData.lups.length > 0) {
            ...
            nav.style.cssText = 'background: white; padding: 0.75rem 1rem; border-radius: 8px; border: 1px solid #e2e8f0; margin-bottom: 1.5rem; display: flex; gap: 0.5rem; overflow-x: acao; white-space: nowrap; align-itemes: center; grid-column: span 2;';
            ...
            opData.lups.forEach((lup, i) => {
                ...
                link.onclick = () => {
                    ...
                    if (el) el.scrollIntoView({ behacaor: 'smooth' });
                };
        ...
        container.style.gridTemplateColumes = '1fr 1fr';
        ...
        (opData.lups || []).forEach((lup, index) => {
            ...
            const colorAtencacao = '#94a3b8'; // gray
            const colorMelhoria = '#e3382c'; // red

            card.innerHTML = `
                <!-- 1. Header (Compact) -->
                <div style="display: grid; grid-template-columes: 1fr 1fr; gap: 0.75rem; margin-bottom: 1rem; border-bottom: 1px solid #e2e8f0; padding-bottom: 0.75rem;">
                    <div>
                        <label style="display:block; font-size: 0.5rem; font-weight: 800; color: #64748b; text-transform: uppercase;">Unidade / Depto</label>
                        <div style="display:flex; align-itemes:center; gap:0.25rem;">
                            ...
                            <input type="text" value="${lup.depto || ''}" oninput="ReportApp.updateLupField(${index}, 'depto', this.value)" style="width:45%; border:none; background:transparent; font-weight:700; color:#1e293b; font-size:0.75rem;" placeholder="Ãrea">
                        </div>
                    </div>
                    <div style="display: flex; justify-content: flex-end; align-itemes: center; gap: 0.5rem;">
                ...
                <div style="display: flex; justify-content: space-between; margin-bottom: 1.25rem; background: rgba(255,255,255,0.7); padding: 0.5rem; border-radius: 4px; border: 1px solid #edf2f7;">
                    <div style="display: flex; align-itemes: center; gap: 0.4rem; cursor: pointer;" onclick="ReportApp.updateLupField(${index}, 'tipo', 'basico')">
                        <div style="width: 14px; height: 14px; border-radius: 50%; border: 2px solid ${colorBasico}; display:flex; align-itemes:center; justify-content:center;">
                            <div style="width: 6px; height: 6px; border-radius: 50%; background: ${lup.tipo === 'basico' ? colorBasico : 'transparent'};"></div>
                        </div>
                        <span style="font-size: 0.6rem; font-weight: 700; color: ${lup.tipo === 'basico' ? colorBasico : '#64748b'};">BÃ¡sico</span>
                    </div>
                    <div style="display: flex; align-itemes: center; gap: 0.4rem; cursor: pointer;" onclick="ReportApp.updateLupField(${index}, 'tipo', 'atencacao')">
                        <div style="width: 14px; height: 14px; border-radius: 50%; border: 2px solid ${colorAtencacao}; display:flex; align-itemes:center; justify-content:center;">
                            <div style="width: 6px; height: 6px; border-radius: 50%; background: ${lup.tipo === 'atencacao' ? colorAtencacao : 'transparent'};"></div>
                        </div>
                        <span style="font-size: 0.6rem; font-weight: 700; color: ${lup.tipo === 'atencacao' ? '#1e293b' : '#64748b'};">AtenÃ§Ã£o</span>
                    </div>
                    <div style="display: flex; align-itemes: center; gap: 0.4rem; cursor: pointer;" onclick="ReportApp.updateLupField(${index}, 'tipo', 'melhoria')">
                        <div style="width: 14px; height: 14px; border-radius: 50%; border: 2px solid ${colorMelhoria}; display:flex; align-itemes:center; justify-content:center;">
                ...
                <div style="margin-bottom: 1.25rem; border-left: 3px solid #142245; padding-left: 0.75rem;">
                    <input type="text" value="${lup.titulo || ''}" oninput="ReportApp.updateLupField(${index}, 'titulo', this.value)" placeholder="TÃTULO DA LIÃ‡ÃƒO" style="width:100%; border:none; background:transparent; font-weight:800; font-size:1.1rem; color:#142245; margin-bottom: 0.15rem; outline:none;" id="lup_title_input_${index}">
                    <p style="font-size:0.6rem; color:#64748b; font-weight:600; text-transform:uppercase; letter-spacing:0.02em;">PadrÃ£o Esperado vs Desvio</p>
                </div>

                <!-- 4. Comparison Columes (Vertication Small, Grid here) -->
                <div style="display: grid; grid-template-columes: 1fr 1fr; gap: 1rem; margin-bottom: 1.25rem;">
                    <!-- Left: Desvio -->
                    <div style="background: white; border-radius: 6px; border: 1px solid #e2e8f0; border-left: 4px solid #e3382c; overflow: hidden; display: flex; flex-direction: column;">
                        <div style="height: 180px; background: #f8fafc; cursor: pointer; display: flex; align-itemes: center; justify-content: center; position: relative;">
                            ${lup.imgErrado ? `
                                ...
                                <button onclick="event.stopPropagation(); ReportApp.openLightboxLup(${index})" style="position:acaolute; top:8px; right:8px; background:rgba(0,0,0,0.6); color:white; border:none; border-radius:4px; width:28px; height:28px; display:flex; align-itemes:center; justify-content:center; cursor:pointer;" title="Ampliar LUP"><i class="fas fa-search-plus"></i></button>
                            ` : `<div style="text-align:center; color:#cbd5e1; width:100%; height:100%; display:flex; flex-direction:column; align-itemes:center; justify-content:center;" onclick="document.getElementById('lup_file_errado_${index}').click()"><i class="fas fa-camera fa-2x"></i><br><span style="font-size:0.5rem; font-weight:800;">DESVIO</span></div>`}
                    ...
                    <div style="background: white; border-radius: 6px; border: 1px solid #e2e8f0; border-left: 4px solid #10b981; overflow: hidden; display: flex; flex-direction: column;">
                        <div style="height: 180px; background: #f8fafc; cursor: pointer; display: flex; align-itemes: center; justify-content: center; position: relative;">
                            ${lup.imgCerto ? `
                                ...
                                <button onclick="event.stopPropagation(); ReportApp.openLightboxLup(${index})" style="position:acaolute; top:8px; right:8px; background:rgba(0,0,0,0.6); color:white; border:none; border-radius:4px; width:28px; height:28px; display:flex; align-itemes:center; justify-content:center; cursor:pointer;" title="Ampliar LUP"><i class="fas fa-search-plus"></i></button>
                            ` : `<div style="text-align:center; color:#cbd5e1; width:100%; height:100%; display:flex; flex-direction:column; align-itemes:center; justify-content:center;" onclick="document.getElementById('lup_file_certo_${index}').click()"><i class="fas fa-camera fa-2x"></i><br><span style="font-size:0.5rem; font-weight:800;">PADRÃƒO</span></div>`}
                ...
                <div style="background: white; border-radius: 6px; padding: 0.75rem; border: 1px solid #e2e8f0; margin-bottom: 1.5rem;">
                    <div style="border-bottom: 1px solid #edf2f7; padding-bottom: 0.5rem; margin-bottom: 0.5rem;">
                        <h5 style="font-size: 0.55rem; font-weight: 900; color: #64748b; text-transform: uppercase; margin:0 0 0.25rem 0; display:flex; align-itemes:center; gap:0.25rem;"><i class="fas fa-microscope"></i> AnÃ¡lise</h5>
                        <textarea oninput="ReportApp.updateLupField(${index}, 'analise', this.value)" style="width:100%; border:none; background:transparent; font-size:0.7rem; color:#475569; min-height:45px; resize:none; line-height:1.3; outline:none;" placeholder="AnÃ¡lise tÃ©cnica...">${lup.analise || ''}</textarea>
                    </div>
                    <div>
                        <h5 style="font-size: 0.55rem; font-weight: 900; color: #142245; text-transform: uppercase; margin:0 0 0.25rem 0; display:flex; align-itemes:center; gap:0.25rem;"><i class="fas fa-bullseye"></i> AÃ§Ã£o/padraonizaÃ§Ã£o</h5>
                        <textarea oninput="ReportApp.updateLupField(${index}, 'acacao', this.value)" style="width:100%; border:none; background:transparent; font-size:0.7rem; color:#142245; min-height:45px; resize:none; line-height:1.3; outline:none;" placeholder="AÃ§Ã£o implementada...">${lup.acacao || ''}</textarea>
                    </div>
                </div>

                <!-- 6. Footer (Mini) -->
                <div style="border-top: 1px solid #e2e8f0; padding-top: 0.75rem; margin-top: acao;">
                    <div style="display: grid; grid-template-columes: 1fr 1fr; gap:0.5rem; margin-bottom: 0.5rem;">
                        <div>
                            <label style="display:block; font-size: 0.45rem; font-weight: 800; color: #94a3b8; text-transform: uppercase;">Doc: LUP SImes</label>
                            <input type="text" value="${lup.codigo || ''}" oninput="ReportApp.updateLupField(${index}, 'codigo', this.value)" style="width:100%; border:none; background:transparent; font-weight:700; font-size:0.6rem; color:#1e293b;" placeholder="CÃ³digo">
                        </div>
                        <div style="text-align: right;">
                            <label style="display:block; font-size: 0.45rem; font-weight: 800; color: #94a3b8; text-transform: uppercase;">ResponsÃ¡vel</label>
                    ...
                    <div style="display: flex; justify-content: space-between; align-itemes: flex-end;">
                        <span style="font-size: 0.5rem; font-weight: 700; color: #cbd5e1;">VersÃ£o: ${lup.versacao || '1.0'}</span>
                        <img src="data:image/png;base64," style="height: 20px; opacity: 0.8;" onerror="this.src=""">
                    </div>
                </div>
            `;
            container.appendChild(card);
        ...
        try {
            ...
            setTimeout(() => {
                ...
                if (container && container.lastChild) {
                    container.lastChild.scrollIntoView({ behacaor: 'smooth', block: 'center' });
                }
            }, 100);
    ...
    handleLupImage: async function (idx, field, input) {
        if (input.files && input.files[0]) {
            ...
            try {
                ...
                console.error("Erro acao processar imagem da LUP:", err);
                console.error("Erro acao processar imagem."); }
        }
    },
    ...
    renderMelhoriaCards: function () {
        ...
        melhorias.forEach((m, index) => {
            ...
            card.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-itemes: flex-start; margin-bottom: 1.5rem;">
                    <div style="flex: 1;">
                         <input type="text" value="${m.titulo || ''}" oninput="ReportApp.updateMelhoriaField(${index}, 'titulo', this.value)" 
                                ...
                                placeholder="TÃTULO DA MELHORIA">
                         <div style="font-size: 0.65rem; color: #64748b; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em;">Registro de MelhoriationtÃ­nua</div>
                    </div>
                ...
                <div style="display: grid; grid-template-columes: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem;">
                    <!-- Lado Ruim (Antes) -->
                    <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                        <div style="font-size: 0.7rem; font-weight: 800; color: #e3382c; text-transform: uppercase; display: flex; align-itemes: center; gap: 0.4rem;">
                            <i class="fas fa-times-circle"></i> SITUAÃ‡ÃƒO ANTERIOR (O QUE ESTAVA RUIM)
                        </div>
                        <div style="height: 200px; background: #f8fafc; border: 2px dashed #e2e8f0; border-radius: 8px; cursor: pointer; display: flex; align-itemes: center; justify-content: center; position: relative; overflow: hidden;">
                            ${m.imgAntes ? `
                                ...
                                <button onclick="event.stopPropagation(); ReportApp.openLightboxMelhoria(${index})" style="position:acaolute; top:8px; right:8px; background:rgba(0,0,0,0.6); color:white; border:none; border-radius:4px; width:28px; height:28px; display:flex; align-itemes:center; justify-content:center; cursor:pointer;" title="Ampliar Melhoria"><i class="fas fa-search-plus"></i></button>
                            ` : `<div style="text-align:center; color:#cbd5e1; width:100%; height:100%; display:flex; flex-direction:column; align-itemes:center; justify-content:center;" onclick="document.getElementById('melhoria_file_antes_${index}').click()"><i class="fas fa-camera fa-2x"></i><br><span style="font-size:0.6rem; font-weight:800; margin-top:5px; display:block;">FOTO ANTES</span></div>`}
                        ...
                        <textarea oninput="ReportApp.updateMelhoriaField(${index}, 'descAntes', this.value)" 
                                  ...
                                  placeholder="Descreva o problema ou situaÃ§Ã£o anterior...">${m.descAntes || ''}</textarea>
                    </div>

                    <!-- Lado Bom (Depois) -->
                    <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                        <div style="font-size: 0.7rem; font-weight: 800; color: #10b981; text-transform: uppercase; display: flex; align-itemes: center; gap: 0.4rem;">
                            <i class="fas fa-check-circle"></i> MELHORIA REALIZADA (FOTO DacaoLUÃ‡ÃƒO)
                        </div>
                        <div style="height: 200px; background: #f8fafc; border: 2px dashed #e2e8f0; border-radius: 8px; cursor: pointer; display: flex; align-itemes: center; justify-content: center; position: relative; overflow: hidden;">
                            ${m.imgDepois ? `
                ...
                <div style="border-top: 1px solid #e2e8f0; padding-top: 1rem; display: flex; justify-content: space-between; align-itemes: center;">
                    <div style="display: flex; gap: 1rem; align-itemes: center;">
                        <div style="display: flex; flex-direction: column;">
                            <label style="font-size: 0.55rem; font-weight: 800; color: #94a3b8; text-transform: uppercase;">Data de ConclusÃ£o</label>
                        ...
                        <div style="display: flex; flex-direction: column;">
                            <label style="font-size: 0.55rem; font-weight: 800; color: #94a3b8; text-transform: uppercase;">ResponsÃ¡vel</label>
                    ...
                    <img src="data:image/png;base64," style="height: 25px; opacity: 0.8;" onerror="this.src="data:image/png;base64,"">
                </div>
            `;
            container.appendChild(card);
        });
    ...
    handleMelhoriaImage: async function (idx, field, input) {
        if (input.files && input.files[0]) {
            ...
            try {
                ...
                console.error("Erro acao processar imagem da melhoria:", err);
                console.error("Erro acao processar imagem."); }
        }
    },
    ...
    handleImageUpload: async function (event, index) {
        ...
        try {
            ...
            console.error("Erro acao processar imagem de seguranÃ§a:", err);
            console.error("Erro acao processar imagem."); }
    },
    ...
    _prepareForExport: function(element) {
        ...
        inputs.forEach(input => {
            ...
            } else if (input.type === 'racao' || input.type === 'checkbox') {
    ...
    downloadOnePagePDF: function () { const element = document.getElementById('reportElement'); this._prepareForExport(element); const controlBar = document.querySelector('.report-control-bar'); if(controlBar) controlBar.style.display = 'none'; try { if (typeof html2pdf === 'undefined') { console.error('html2pdf is messing'); if(controlBar) controlBar.style.display = 'flex'; return; } const opt = { margin: [5, 5, 5, 5], filename: `${this.currentOp} - ${this.currentDateYMD}.pdf`, image: { type: 'jpeg', quality: 1.0 }, html2canvas: { scale: 2, useCORS: true, letterRendering: true, backgroundColor: '#f1f5f9', scrollY: 0 }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'landscape' } }; html2pdf().set(opt).from(element).save().then(() => { if(controlBar) controlBar.style.display = 'flex'; }).catch(err => { console.error(err); if(controlBar) controlBar.style.display = 'flex'; }); } catch(e) { console.error(e); if(controlBar) controlBar.style.display = 'flex'; } },

    downloadOnePageImage: function () { const element = document.getElementById('reportElement'); this._prepareForExport(element); const controlBar = document.querySelector('.report-control-bar'); if(controlBar) controlBar.style.display = 'none'; try { if (typeof html2canvas === 'undefined') { console.error('html2canvas is messing'); if(controlBar) controlBar.style.display = 'flex'; return; } html2canvas(element, { scale: 2, useCORS: true, backgroundColor: '#f1f5f9', scrollY: 0 }).then(canvas => { const link = document.createElement('a'); link.download = `${this.currentOp} - ${this.currentDateYMD}.png`; link.href = canvas.toDataURL('image/png', 1.0); link.click(); if(controlBar) controlBar.style.display = 'flex'; }).catch(err => { console.error(err); if(controlBar) controlBar.style.display = 'flex'; }); } catch(e) { console.error(e); if(controlBar) controlBar.style.display = 'flex'; } },

    downloadProfessionalPDF: function () { const element = document.getElementById('reportElement'); this._prepareForExport(element); const existingLines = element.querySelectorAll('.preview-break-line'); existingLines.forEach(l => l.style.display = 'none'); const controlBar = document.querySelector('.report-control-bar'); if(controlBar) controlBar.style.display = 'none'; try { if (typeof html2pdf === 'undefined') { console.error('html2pdf is messing'); if(controlBar) controlBar.style.display = 'flex'; existingLines.forEach(l => l.style.display = 'block'); return; } const opt = { margin: [10, 10, 10, 10], filename: `${this.currentOp} - ${this.currentDateYMD} (Profissional).pdf`, image: { type: 'jpeg', quality: 1.0 }, html2canvas: { scale: 2, useCORS: true, letterRendering: true, backgroundColor: '#f1f5f9', scrollY: 0 }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'landscape' }, pagebreak: { mode: ['css', 'legacy'] } }; html2pdf().set(opt).from(element).save().then(() => { if(controlBar) controlBar.style.display = 'flex'; existingLines.forEach(l => l.style.display = 'block'); }).catch(err => { console.error(err); if(controlBar) controlBar.style.display = 'flex'; existingLines.forEach(l => l.style.display = 'block'); }); } catch(e) { console.error(e); if(controlBar) controlBar.style.display = 'flex'; existingLines.forEach(l => l.style.display = 'block'); } },
    ...
    downloadPDF: function () {
        ...
        // Verifica se a bibliotecacaoi carregada
        if (typeof html2pdf === 'undefined') {
            console.warn("A biblioteca de PDF ainda estÃ¡ carregacao.");
        ...
        html2pdf().set(opt).from(reportElement).save().then(() => {
            ...
            console.error('Erro acao geracao PDF:', err);
            if (controls) controls.style.display = oldDisplay;
            reportElement.style.transform = originalTransform;
            console.error('Falha silenciosa na geracao do PDF');
        });
    },
    ...
    migrateExistingImages: async function () {
        console.log("Iniciacao escaneamento e migraÃ§Ã£o de imagens grandes em background...");
        let migrated = false;
        for (let opName in this.data) {
            const op = this.data[opName];
                ...
                if (daily.safety && daily.safety.images) {
                    for (let i = 0; i < daily.safety.images.length; i++) {
                        ...
                        if (img && img.startsWith('data:image/') && img.length > 250000) {
                            try {
                                console.log(`Comprimindo imagem antiga de SeguranÃ§a da filial ${opName} em ${dateYMD}...`);
                                daily.safety.images[i] = await this.compressBase64Image(img);
                                migrated = true;
                            } catch (e) { 
                                console.error("Erro migracao imagem de seguranÃ§a:", e); }
                        }
                    }
                }
                ...
                if (daily.lups) {
                    for (let i = 0; i < daily.lups.length; i++) {
                        ...
                        if (lup.imgErrado && lup.imgErrado.startsWith('data:image/') && lup.imgErrado.length > 250000) {
                            try {
                                console.log(`Comprimindo imagem antigacao Desvio de LUP #${i+1} da filial ${opName} em ${dateYMD}...`);
                                lup.imgErrado = await this.compressBase64Image(lup.imgErrado);
                                migrated = true;
                            } catch (e) { 
                                console.error("Erro migracao imgErrado de LUP:", e); }
                        }
                        if (lup.imgCerto && lup.imgCerto.startsWith('data:image/') && lup.imgCerto.length > 250000) {
                            try {
                                console.log(`Comprimindo imagem antigacao PadrÃ£o de LUP #${i+1} da filial ${opName} em ${dateYMD}...`);
                                lup.imgCerto = await this.compressBase64Image(lup.imgCerto);
                                migrated = true;
                            } catch (e) { 
                                console.error("Erro migracao imgCerto de LUP:", e); }
                        }
                    }
                }
                    ...
                    for (let i = 0; i < daily.melhorias.length; i++) {
                        ...
                        if (m.imgAntes && m.imgAntes.startsWith('data:image/') && m.imgAntes.length > 250000) {
                            try {
                                ...
                                console.error("Erro migracao imgAntes de Melhoria:", e); }
                        }
                        if (m.imgDepois && m.imgDepois.startsWith('data:image/') && m.imgDepois.length > 250000) {
                            try {
                                ...
                                console.error("Erro migracao imgDepois de Melhoria:", e); }
                        }
                    }
                }
            }
        ...
        if (migrated) {
            console.log("MigraÃ§Ã£o/CompressÃ£o concluÃ­dacaomesucesso! Atualizacao localStorage e Firebase...");
            // Save full operations so historical date changes are successfully propagated
            this.saveToLocalStorage(false, true);
            this.render();
        } else {
            console.log("Escaneamento de background concluÃ­do. Nenhuma imagem antiga precisou de compressÃ£o.");
        }
    }
